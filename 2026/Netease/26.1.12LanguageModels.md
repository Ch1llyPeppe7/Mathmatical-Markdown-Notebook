...
令$$\Phi(q)=(e^{i\omega_1 q},...,e^{i\omega_mq})$$则$$\langle \Phi(q_1),\Phi(q_2)\rangle_{\mathbb{C}}=\sum_{k=1}^m e^{i\omega_k q_1}\overline{e^{i\omega_k q_2}}=\sum_{k=1}^m e^{i\omega_k (q_1-q_2)}=\sum_{k=1}^m e^{i\omega_k \Delta q}$$其中$\Delta q = q_1 - q_2$。因此，傅里叶位置编码对应的核函数为$$K(\Delta q)=\sum_{k=1}^m e^{i\omega_k \Delta q}$$

这是一个离散版本的Bochner表示，其中测度$\mu$是离散测度：$$\mu=\sum_{k=1}^m \delta_{\omega_k}$$即$\mu$在频率点$\{\omega_1,\ldots,\omega_m\}$上各有一个单位质量。
...
考虑傅里叶位置编码的核函数$$K(\Delta q)=\sum_{k=1}^m e^{i\omega_k \Delta q}=\sum_{k=1}^m \cos(\omega_k \Delta q) + i\sum_{k=1}^m \sin(\omega_k \Delta q)$$

其实部为$$\text{Re}(K(\Delta q))=\sum_{k=1}^m \cos(\omega_k \Delta q)$$

频率数列$\{\omega_1,\ldots,\omega_m\}$的选择直接影响这个震荡积分的形态：

1. **频率分布的影响**：
   - 如果$\{\omega_k\}$均匀分布（如$\omega_k = \frac{2\pi k}{m}$），则$K(\Delta q)$呈现**周期性**，周期为$m$
   - 如果$\{\omega_k\}$按几何级数分布（如$\omega_k = \omega_0 \cdot 2^{-k}$），则$K(\Delta q)$呈现**多尺度震荡**，低频分量控制大尺度结构，高频分量控制细节

2. **频率范围的影响**：
   - 如果$\{\omega_k\}$集中在低频（$\omega_k \ll 1$），则$K(\Delta q)$变化缓慢，对位置差异不敏感
   - 如果$\{\omega_k\}$包含高频分量（$\omega_k \gg 1$），则$K(\Delta q)$变化剧烈，对位置差异高度敏感

3. **频率密度的影响**：
   - 如果$\{\omega_k\}$密集分布，则$K(\Delta q)$更接近连续积分$\int e^{i\omega \Delta q}d\mu(\omega)$
   - 如果$\{\omega_k\}$稀疏分布，则$K(\Delta q)$呈现明显的**拍频效应**（beating），即不同频率分量之间的干涉

</blockquote>

<blockquote class="property">

**实部与$\Delta q$的关系：频率数列的影响**

对于傅里叶位置编码的核函数实部$$\text{Re}(K(\Delta q))=\sum_{k=1}^m \cos(\omega_k \Delta q)$$

我们分析$\{\omega_k\}$的选择如何影响实部与$\Delta q$的关系：

1. **均匀频率分布**（$\omega_k = \frac{2\pi k}{m}$）：
   $$\text{Re}(K(\Delta q))=\sum_{k=1}^m \cos\left(\frac{2\pi k}{m}\Delta q\right)$$
   - 当$\Delta q = 0$时，$\text{Re}(K(0)) = m$（最大值）
   - 当$\Delta q = m$时，$\text{Re}(K(m)) = \sum_{k=1}^m \cos(2\pi k) = m$（周期性）
   - 实部呈现**周期性衰减**，周期为$m$

2. **几何级数频率分布**（$\omega_k = \omega_0 \cdot 2^{-k}$）：
   $$\text{Re}(K(\Delta q))=\sum_{k=1}^m \cos(\omega_0 \cdot 2^{-k} \Delta q)$$
   - 低频分量（$k$大）控制大尺度行为：当$\Delta q$较小时，主要贡献来自高频分量
   - 高频分量（$k$小）控制细节：当$\Delta q$较大时，主要贡献来自低频分量
   - 实部呈现**多尺度震荡**，不同频率分量在不同$\Delta q$范围内起主导作用

3. **对数均匀频率分布**（$\omega_k = \omega_{\min} \cdot \left(\frac{\omega_{\max}}{\omega_{\min}}\right)^{\frac{k-1}{m-1}}$）：
   $$\text{Re}(K(\Delta q))=\sum_{k=1}^m \cos\left(\omega_{\min} \cdot \left(\frac{\omega_{\max}}{\omega_{\min}}\right)^{\frac{k-1}{m-1}} \Delta q\right)$$
   - 在$\Delta q$较小时，高频分量起主导作用，实部快速衰减
   - 在$\Delta q$较大时，低频分量起主导作用，实部缓慢衰减
   - 实部呈现**对数尺度上的均匀衰减**

4. **频率间隔的影响**：
   - 如果$\omega_{k+1} - \omega_k$较小（密集分布），则不同频率分量的干涉更平滑
   - 如果$\omega_{k+1} - \omega_k$较大（稀疏分布），则会出现明显的**拍频现象**：
     $$\cos(\omega_k \Delta q) + \cos(\omega_{k+1} \Delta q) = 2\cos\left(\frac{\omega_k+\omega_{k+1}}{2}\Delta q\right)\cos\left(\frac{\omega_{k+1}-\omega_k}{2}\Delta q\right)$$
     其中第二个余弦项产生低频包络，频率为$\frac{\omega_{k+1}-\omega_k}{2}$

5. **幂函数频率分布**（$\omega_k = \left(\frac{k}{m}\right)^a$，其中$d=2m$，$\omega_k = \left(\frac{2k}{d}\right)^a = \left(\frac{k}{m}\right)^a$）：
   $$\text{Re}(K(\Delta q))=\sum_{k=1}^m \cos\left(\left(\frac{k}{m}\right)^a \Delta q\right)$$
   
   频率间隔为：
   $$\Delta\omega_k = \omega_{k+1} - \omega_k = \left(\frac{k+1}{m}\right)^a - \left(\frac{k}{m}\right)^a = \frac{1}{m^a}\left[(k+1)^a - k^a\right]$$
   
   当$k$较大时，使用一阶近似：
   $$\Delta\omega_k \approx \frac{a}{m^a} \cdot k^{a-1}$$
   
   **不同$a$值的影响**：
   - **$a=1$（线性分布）**：$\omega_k = \frac{k}{m}$，$\Delta\omega_k = \frac{1}{m}$（常数间隔）
     - 频率均匀分布，类似均匀频率分布
     - 实部呈现周期性衰减
   
   - **$a>1$（超线性增长）**：$\Delta\omega_k \propto k^{a-1}$（递增间隔）
     - 低频区域频率密集，高频区域频率稀疏
     - 在$\Delta q$较小时，多个低频分量叠加，实部衰减较慢
     - 在$\Delta q$较大时，高频分量稀疏，实部呈现不规则震荡
   
   - **$0<a<1$（次线性增长）**：$\Delta\omega_k \propto k^{a-1}$（递减间隔）
     - 低频区域频率稀疏，高频区域频率密集
     - 在$\Delta q$较小时，低频分量稀疏，实部快速衰减
     - 在$\Delta q$较大时，多个高频分量叠加，实部呈现高频震荡
   
   - **$a<0$（递减）**：$\omega_k = \left(\frac{k}{m}\right)^a = \left(\frac{m}{k}\right)^{|a|}$（频率递减）
     - 类似几何级数分布，但衰减速度由$|a|$控制
     - 当$|a|$较大时，频率快速衰减，主要贡献来自前几个低频分量
...
对于傅里叶位置编码的核函数$$K(\Delta q)=\sum_{k=1}^m e^{i\omega_k \Delta q}$$

当$\Delta q \to \infty$时，如果频率数列$\{\omega_k\}$满足某些条件，则$K(\Delta q)$的渐近行为由频率分布决定：

1. **如果$\{\omega_k\}$线性无关**（即不存在非零整数向量$(n_1,\ldots,n_m)$使得$\sum_{k=1}^m n_k \omega_k = 0$），则$K(\Delta q)$在$\Delta q \to \infty$时**不收敛**，呈现准周期震荡。

2. **如果$\{\omega_k\}$有理相关**（即存在非零整数向量$(n_1,\ldots,n_m)$使得$\sum_{k=1}^m n_k \omega_k = 0$），则$K(\Delta q)$是**周期函数**，周期为所有$\omega_k$的最大公约数的倒数。

3. **如果$\{\omega_k\}$在某个区间$[\omega_{\min},\omega_{\max}]$上**密集分布**（如$\omega_k = \omega_{\min} + \frac{k-1}{m-1}(\omega_{\max}-\omega_{\min})$），则当$m \to \infty$时，$K(\Delta q)$收敛到连续积分：
   $$K(\Delta q) \to \int_{\omega_{\min}}^{\omega_{\max}} e^{i\omega \Delta q} d\omega = \frac{e^{i\omega_{\max}\Delta q} - e^{i\omega_{\min}\Delta q}}{i\Delta q}$$
   其实部为$$\text{Re}(K(\Delta q)) \to \frac{\sin(\omega_{\max}\Delta q) - \sin(\omega_{\min}\Delta q)}{\Delta q}$$
   呈现**$\frac{1}{\Delta q}$衰减**。
...
1. **Transformer原始位置编码**（$\omega_k = \frac{1}{10000^{2k/d}}$，其中$d$为维度）：
   - 频率按几何级数递减
   - 实部在$\Delta q$较小时主要由高频分量贡献，快速震荡
   - 实部在$\Delta q$较大时主要由低频分量贡献，缓慢震荡
   - 适合捕捉**多尺度位置关系**

2. **均匀频率分布**（$\omega_k = \frac{2\pi k}{m}$）：
   - 频率均匀分布，周期为$m$
   - 实部呈现周期性，周期为$m$
   - 适合捕捉**周期性位置模式**（如时间序列中的周期性）

3. **对数均匀频率分布**（$\omega_k = 2^{-k}$）：
   - 频率在对数尺度上均匀分布
   - 实部在不同$\Delta q$范围内由不同频率分量主导
   - 适合捕捉**多分辨率位置关系**

4. **随机频率分布**（$\omega_k$从某个分布中采样）：
   - 频率随机分布，可能导致拍频效应
   - 实部呈现**不规则震荡**
   - 适合捕捉**非结构化位置关系**

5. **幂函数频率分布**（$\omega_k = \left(\frac{2k}{d}\right)^a$，其中$d=2m$）：
   - 频率按幂函数分布，指数$a$控制分布特性
   - 当$a=1$时，退化为线性分布：$\omega_k = \frac{k}{m}$
   - 当$a>1$时，频率增长加速，高频分量更密集
   - 当$0<a<1$时，频率增长减速，低频分量更密集
   - 当$a<0$时，频率递减，类似几何级数
...
在实际应用中，频率数列$\{\omega_k\}$的选择需要平衡以下因素：

1. **表达能力**：需要覆盖足够宽的频率范围，以捕捉不同尺度的位置关系
2. **计算效率**：频率数量$m$不能太大，否则计算成本过高
3. **数值稳定性**：避免频率过于接近导致的数值问题
4. **任务特性**：根据具体任务（如自然语言处理、时间序列预测）选择合适的频率分布

**建议**：
- 对于**长序列**（如文档级任务），使用几何级数或对数均匀分布，强调低频分量
- 对于**短序列**（如句子级任务），使用均匀分布或高频密集分布，强调高频分量
- 对于**周期性数据**（如时间序列），使用均匀频率分布，匹配数据的周期性
...
对于幂函数频率分布$\omega_k = \left(\frac{k}{m}\right)^a$，当$m \to \infty$时，核函数$K(\Delta q)$的渐近行为取决于指数$a$：

1. **当$a > 0$时**，频率范围$[\omega_1, \omega_m] = \left[\frac{1}{m^a}, 1\right]$，当$m \to \infty$时：
   - 如果$a > 1$：$\omega_1 = \frac{1}{m^a} \to 0$，频率范围扩展到$[0, 1]$
   - 如果$a = 1$：$\omega_1 = \frac{1}{m} \to 0$，频率范围扩展到$[0, 1]$
   - 如果$0 < a < 1$：$\omega_1 = \frac{1}{m^a} \to 0$，但收敛速度较慢

2. **连续极限**（$m \to \infty$）：
   当$m \to \infty$时，离散和$\sum_{k=1}^m e^{i\omega_k \Delta q}$收敛到积分：
   $$K(\Delta q) \to m \int_0^1 e^{i x^a \Delta q} dx$$
   
   通过变量替换$u = x^a$，$x = u^{1/a}$，$dx = \frac{1}{a} u^{1/a - 1} du$：
   $$K(\Delta q) \to \frac{m}{a} \int_0^1 u^{1/a - 1} e^{i u \Delta q} du$$
   
   这是一个**分数阶积分**，其渐近行为由$a$决定：
   - 当$\Delta q \to 0$时：$K(\Delta q) \approx m$（所有频率分量同相）
   - 当$\Delta q \to \infty$时：使用**稳相法**（method of stationary phase），主要贡献来自$u \approx 0$附近，得到：
     $$K(\Delta q) \sim \frac{m}{a} \cdot \frac{\Gamma(1/a)}{(i\Delta q)^{1/a}}$$
     其中$\Gamma$是Gamma函数，衰减速度为$\frac{1}{(\Delta q)^{1/a}}$

3. **实部的渐近行为**：
   $$\text{Re}(K(\Delta q)) \sim \frac{m}{a} \cdot \frac{\Gamma(1/a)}{(\Delta q)^{1/a}} \cdot \cos\left(\frac{\pi}{2a}\right)$$
   
   **关键观察**：
   - 当$a > 1$时：衰减速度$\frac{1}{(\Delta q)^{1/a}} < \frac{1}{\Delta q}$（慢衰减）
   - 当$a = 1$时：衰减速度$\frac{1}{\Delta q}$（标准衰减）
   - 当$0 < a < 1$时：衰减速度$\frac{1}{(\Delta q)^{1/a}} > \frac{1}{\Delta q}$（快衰减）
...
考虑$\omega_k = \left(\frac{2k}{d}\right)^a$，其中$d=2m$，即$\omega_k = \left(\frac{k}{m}\right)^a$：

1. **$a=2$（平方分布）**：
   - $\omega_k = \left(\frac{k}{m}\right)^2$，频率间隔$\Delta\omega_k \approx \frac{2k}{m^2}$（线性增长）
   - 低频区域（$k$小）频率密集，高频区域（$k$大）频率稀疏
   - 实部在$\Delta q$较小时衰减缓慢（多个低频分量叠加）
   - 实部在$\Delta q$较大时呈现不规则震荡（高频分量稀疏）
   - **适用场景**：需要强调局部位置关系，对远距离位置关系不敏感

2. **$a=0.5$（平方根分布）**：
   - $\omega_k = \sqrt{\frac{k}{m}}$，频率间隔$\Delta\omega_k \approx \frac{1}{2\sqrt{km}}$（递减）
   - 低频区域（$k$小）频率稀疏，高频区域（$k$大）频率密集
   - 实部在$\Delta q$较小时快速衰减（低频分量稀疏）
   - 实部在$\Delta q$较大时呈现高频震荡（多个高频分量叠加）
   - **适用场景**：需要捕捉细节位置关系，对全局位置关系不敏感

3. **$a=-1$（倒数分布）**：
   - $\omega_k = \frac{m}{k}$，频率递减，类似几何级数
   - 频率间隔$\Delta\omega_k = \frac{m}{k(k+1)} \approx \frac{m}{k^2}$（快速递减）
   - 主要贡献来自前几个低频分量
   - 实部呈现类似几何级数分布的行为
   - **适用场景**：需要强调全局位置关系，对局部细节不敏感

4. **$a=1$（线性分布）**：
   - $\omega_k = \frac{k}{m}$，频率均匀分布
   - 频率间隔$\Delta\omega_k = \frac{1}{m}$（常数）
   - 实部呈现周期性衰减，周期为$m$
   - **适用场景**：需要均匀覆盖所有频率范围
...
对于$\omega_k = \left(\frac{2k}{d}\right)^a$形式的频率分布，指数$a$的选择需要根据任务特性：

1. **任务对位置敏感性的要求**：
   - **局部敏感任务**（如词性标注、命名实体识别）：选择$a > 1$，强调低频分量，捕捉局部模式
   - **全局敏感任务**（如文档分类、长文本理解）：选择$0 < a < 1$或$a < 0$，强调高频分量或快速衰减，捕捉全局模式
   - **平衡任务**（如机器翻译、文本生成）：选择$a \approx 1$，均匀覆盖所有频率

2. **序列长度的考虑**：
   - **短序列**（$L < 100$）：选择$a \geq 1$，避免过度强调低频分量
   - **中等序列**（$100 \leq L < 1000$）：选择$a \approx 1$，平衡各频率分量
   - **长序列**（$L \geq 1000$）：选择$a < 1$，强调高频分量以捕捉细节

3. **计算效率**：
   - 当$a > 1$时，低频分量密集，需要更多计算资源
   - 当$a < 1$时，高频分量密集，但可以通过截断高频分量来加速

4. **数值稳定性**：
   - 当$a$很大时，频率间隔变化剧烈，可能导致数值问题
   - 建议$a \in [-2, 3]$范围内选择
