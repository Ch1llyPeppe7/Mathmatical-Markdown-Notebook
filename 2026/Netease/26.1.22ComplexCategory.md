...
本笔记旨在建立复形范畴（simplicial complex category）与词表构建算法之间的理论桥梁。我们以范畴论为语言，将Byte Pair Encoding (BPE) 和逆向分裂（inverse splitting）统一理解为复形范畴上的余极限与极限操作，从而揭示这两种看似不同的算法在数学结构上的对偶本质。

笔记分为三个主要部分：首先，我们建立基础概念框架，包括单纯形、单纯复形、共轭复形等核心定义，并引入加权复形与有向单纯复形以适应实际应用场景；其次，我们系统阐述复形范畴中的极限与余极限理论，包括积、拉回、余积、推出等核心构造，并探讨函子与伴随对的理论基础；最后，我们将理论应用于BPE与逆向分裂算法，展示如何通过范畴论的视角理解词表构建的本质过程。

特别地，我们引入了代数拓扑中的同调理论与信息论中的渐进均分性（AEP），为逆向分裂算法提供了严格的数学基础。通过共轭复形的概念，我们揭示了BPE与逆向分裂之间的对偶关系：前者在原复形中通过余极限聚合原子为Token，后者在共轭复形中通过极限分解全局模式为极小原子簇。
...
在建立了基础概念框架之后，我们现在转向复形范畴的核心构造：极限与余极限。这些构造不仅是范畴论的核心概念，更是我们理解BPE与逆向分裂算法的关键。极限与余极限分别对应"分解"与"聚合"两种操作，它们通过泛性质保证了构造的唯一性和最优性。
...
在分析BPE算法时，我们发现它本质上是复形范畴上的余极限过程（从原子聚合为Token）。为了设计逆向算法，我们需要找到对偶的极限过程，使得：
- 输入是全局的交互模式（高维单纯形）；
- 输出是极小的原子簇（低维单纯形）；
- 通过拉回构造，在约束条件下分解整体为基元。

这种"分解"的极限过程与BPE的"聚合"余极限过程形成自然的对偶关系。
</blockquote>
...
拉回是积的推广。它不仅在两个复形之间构造积，还要求这个积与第三个复形通过态射"对齐"。这种对齐关系使得拉回能够捕捉"公共子结构"，这正是逆向分裂算法中"极小原子簇"的数学本质。
...
BPE算法通过余极限将原子聚合为Token，而逆向分裂则通过极限将全局交互模式分解为极小原子簇。这两种操作在数学结构上是对偶的：BPE在原复形中进行，而逆向分裂在共轭复形中进行，它们通过"顶点-单纯形反转"形成自然的对偶关系。

<blockquote class="section">

## 逆向分裂作为极限过程

逆向分裂是BPE的对偶算法。如果说BPE是通过余极限"聚合"原子为Token，那么逆向分裂就是通过极限"分解"全局交互模式为极小原子簇。这种对偶关系不仅体现在算法结构上，更体现在它们所操作的复形上：BPE在原复形中进行，而逆向分裂在共轭复形中进行。
...
BPE与逆向分裂分别在原复形与共轭复形中进行，这种结构上的对偶性不仅体现在它们使用的数学工具（余极限与极限）上，更体现在它们所操作的复形结构上。共轭复形通过"顶点-单纯形反转"实现了这种对偶关系，使得两种算法在数学上形成完美的对称。
...
![逆向分裂的拉回构造过程：超图逐步分解](images/figure3_inverse_splitting_pullback.png)
</blockquote>
...
共轭复形关系揭示了BPE与逆向分裂在结构上的对偶性，但更深入的对偶关系体现在它们的性能特征上。BPE通过聚合提升记忆效率，但泛化能力受限；逆向分裂通过分解提升泛化能力，但记忆效率可控。这种性能上的对偶性正是共轭结构在算法设计中的体现。
