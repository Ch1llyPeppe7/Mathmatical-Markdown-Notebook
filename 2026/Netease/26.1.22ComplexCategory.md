...
给定非空顶点集 $V$，一个单纯复形 $K$ 是 $V$ 的有限子集族（其中元素称为单纯形），满足：
...
单纯形 $\sigma$ 的维度定义为其顶点数减1，记为 $\dim \sigma = |\sigma| - 1$；复形 $K$ 的维度为所含单纯形的最大维度，记为 $\dim K = \max\{\dim \sigma \mid \sigma \in K\}$。
</blockquote>
...
设 $K, L$ 为两个单纯复形，顶点集分别为 $V_K, V_L$，函数 $f: V_K \to V_L$ 称为从 $K$ 到 $L$ 的单纯映射，若满足：
对任意 $\sigma = \{v_1, v_2, ..., v_n\} \in K$，其像集 $f(\sigma) = \{f(v_1), f(v_2), ..., f(v_n)\} \in L$（保持单纯形结构）。

单纯映射可自然延拓到全体单纯形上：对 $\sigma \in K$，定义 $f: K \to L$ 为 $f(\sigma) = \{f(v) \mid v \in \sigma\}$，延拓后仍记为 $f$。
</blockquote>

<blockquote class="property">

1. 维度单调性：对任意 $\sigma \in K$，$\dim f(\sigma) \leq \dim \sigma$；
2. 复合封闭性：若 $f: K \to L$、$g: L \to M$ 均为单纯映射，则复合映射 $g \circ f: K \to M$ 也是单纯映射；
3. 恒等映射存在：对任意复形 $K$，存在恒等单纯映射 $\text{id}_K: K \to K$，满足 $\text{id}_K(v) = v$ 对所有 $v \in V_K$ 成立。
</blockquote>
...
极限的范畴论定义
设 $\mathcal{I}$ 为小指标范畴，图表 $\mathcal{D}: \mathcal{I} \to \mathbf{Comp}$，若存在复形 $L \in \text{Ob}(\mathbf{Comp})$ 及一族投影态射 $\{\pi_i: L \to \mathcal{D}(i)\}_{i \in \mathcal{I}}$，满足泛性质：
对任意复形 $X \in \text{Ob}(\mathbf{Comp})$ 及态射族 $\{f_i: X \to \mathcal{D}(i)\}_{i \in \mathcal{I}}$，存在唯一态射 $f: X \to L$，使得对所有 $i \in \mathcal{I}$，$\pi_i \circ f = f_i$，则称 $(L, \{\pi_i\})$ 为图表 $\mathcal{D}$ 的极限，记为 $\lim_{\leftarrow} \mathcal{D}$。
</blockquote>

<blockquote class="example">

核心构造1：积（Product）
对任意两个复形 $K, L \in \text{Ob}(\mathbf{Comp})$，其积 $K \times L$ 定义为：
1. 顶点集 $V_{K \times L} = V_K \times V_L$（顶点的笛卡尔积）；
2. 单纯形族：$\sigma \in K \times L$ 当且仅当 $\sigma$ 是有限子集，且其在 $K$、$L$ 上的投影分别为 $K$、$L$ 中的单纯形。
...
\end{CD}
$$
其中 $\pi_1: K \times L \to K$、$\pi_2: K \times L \to L$ 为投影态射（取顶点的第一、第二分量）。
</blockquote>

<blockquote class="example">

核心构造2：拉回（Pullback）
给定态射 $f: K \to M$、$g: L \to M$，拉回 $K \times_M L$ 是满足 $f \circ \pi_1 = g \circ \pi_2$ 的极限对象，核心交换方块：
$$
\begin{CD}
K \times_M L @>{\pi_1}>> K \\
@V{\pi_2}VV @VV{f}V \\
...
$$
拉回的具体构造：
1. 顶点集 $V_{K \times_M L} = \{(v, w) \in V_K \times V_L \mid f(v) = g(w)\}$；
2. 单纯形族：$\sigma \in K \times_M L$ 当且仅当 $\pi_1(\sigma) \in K$ 且 $\pi_2(\sigma) \in L$。
</blockquote>
...
余极限的范畴论定义
设 $\mathcal{I}$ 为小指标范畴，图表 $\mathcal{D}: \mathcal{I} \to \mathbf{Comp}$，若存在复形 $C \in \text{Ob}(\mathbf{Comp})$ 及一族嵌入态射 $\{\iota_i: \mathcal{D}(i) \to C\}_{i \in \mathcal{I}}$，满足泛性质：
对任意复形 $Y \in \text{Ob}(\mathbf{Comp})$ 及态射族 $\{g_i: \mathcal{D}(i) \to Y\}_{i \in \mathcal{I}}$，存在唯一态射 $g: C \to Y$，使得对所有 $i \in \mathcal{I}$，$g \circ \iota_i = g_i$，则称 $(C, \{\iota_i\})$ 为图表 $\mathcal{D}$ 的余极限，记为 $\lim_{\rightarrow} \mathcal{D}$。
</blockquote>

<blockquote class="example">

核心构造1：余积（Coproduct）
对任意两个复形 $K, L \in \text{Ob}(\mathbf{Comp})$，其余积 $K \sqcup L$ 定义为：
1. 顶点集 $V_{K \sqcup L} = V_K \sqcup V_L$（顶点的无交并）；
2. 单纯形族：$\sigma \in K \sqcup L$ 当且仅当 $\sigma \subseteq V_K$ 且 $\sigma \in K$，或 $\sigma \subseteq V_L$ 且 $\sigma \in L$。
...
\end{CD}
$$
其中 $\iota_1: K \to K \sqcup L$、$\iota_2: L \to K \sqcup L$ 为嵌入态射（顶点的包含映射）。
</blockquote>

<blockquote class="example">

核心构造2：推出（Pushout）
给定态射 $f: M \to K$、$g: M \to L$，推出 $K \sqcup_M L$ 是满足 $\iota_1 \circ f = \iota_2 \circ g$ 的余极限对象，核心交换方块：
$$
\begin{CD}
M @>{f}>> K \\
@V{g}VV @VV{\iota_1}V \\
...
$$
推出的具体构造：
1. 顶点集 $V_{K \sqcup_M L} = (V_K \sqcup V_L)/\sim$（无交并模等价关系：$f(v) \sim g(v)$ 对所有 $v \in V_M$）；
2. 单纯形族：$\sigma \in K \sqcup_M L$ 当且仅当 $\sigma$ 的代表元集是 $K$ 或 $L$ 中的单纯形。
</blockquote>
...
BPE所在的原复形结构
正向BPE的整个过程均在原复形 $K$ 中进行，其与原复形的对应关系：
1. 初始状态：预设原子表对应 $K$ 的0维单纯形（顶点），即 $V_K = S$（原子集合），$K_0 = V_K$（0维单纯形族）；
