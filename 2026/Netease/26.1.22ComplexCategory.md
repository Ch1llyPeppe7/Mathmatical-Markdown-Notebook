...
**从直觉出发**
我们有一个有限点集$V:=\set{v_1,...,v_n}$,我们想找到一族覆盖$\mathcal{C}:=\set{C_1,...,C_m}$,其中$\forall j,C_j\subseteq V$且$m<n$,希望每个点$v_i\in V$可以被**不重复地**表示为$$v_i=\set{j|v_i\in C_j},$$我们称覆盖的**索引**为原子(Atom)。
从直觉上来看，以下规律可以供覆盖族设计参考：
- **覆盖够大**：每个覆盖都尽量承载更多的点，从而减少覆盖的数量
- **点参与较多覆盖**：如果所有点只有一个覆盖，那就退化成m=n的情形；覆盖之间的连通性和泛化潜力有关。
- **覆盖差异要大**：如果所有覆盖之间有大量的重复点，难以满足**唯一性**
</blockquote>
<blockquote class="section">
...
考虑一个无向二部图$\mathcal{G}:=(U,V,R)$,其中交互矩阵$R\in\mathbb{N}^{|U|\times|V|}$，我们假设图是没有自环的，那么邻接矩阵$A$形式如下：
$$A=\begin{pmatrix}
  \mathbf{0}&R\\
  R^T&\mathbf{0}
\end{pmatrix}$$
我们不妨考虑一个关于物品的有向图$\mathcal{G}_1:=(V,R_1)$,其中路径矩阵$R_1\in\mathbb{N}^{|V|\times|V|}$,考虑一个交互序列集合$S$上所有邻接的物品对(1-simplex)$v_i\rightarrow v_j$，我们令$$(R_1)_{ij}=Count(v_i\rightarrow v_j),$$这是一个有向边(1-chain)的权重矩阵（未归一化）我们可以构造它的一对对称版本$R_s$与反对称版本$R_{sk}$$$\begin{align}
  R_s=&R_1+R_1^T\\
  R_{sk}=&R_1-R_1^T,
\end{align}$$
这只是一个朦胧的开始，我们在引入更多强力的概念之后一切就会变得更加清晰。
</blockquote>

<blockquote class="definition">

**Chain Space $\mathcal{C}^1$ over 1-complex(Graph)**
考虑一个有限有向图$\mathcal{G}:=(V,E)$,其上的链空间是边上的标量函数族$$\mathcal{C}^1\cong \mathbb{R}^{|E|}$$$\mathcal{G}$中的每条有向边$e=(i\rightarrow j)$都是一个基向量，
链空间的元素$f\in \mathcal{C}^1$是边上的权重、频率或者强度等等。
$$\delta_0:\mathcal{C}^0\rightarrow \mathcal{C}^1$$
</blockquote>

<blockquote class="theorem">
  
**Hodge Decomposition over $\mathcal{C}^1$**
$$\mathcal{C}^1=im\delta_0\oplus ker\Delta_1\oplus im\delta_1^*$$任何定义在边上的有向流，都可以唯一地拆分成以下三部分
</blockquote>
</blockquote>
</blockquote>
