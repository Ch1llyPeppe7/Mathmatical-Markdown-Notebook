...
<blockquote class="cover-page">
  <div class="cover-title">跳跃连接网络系列研究</div>
...
![图片描述](./Materials/SkipConnection.png)
...
残差网络ResNet实际上定义了一种新的系统,特征映射退化为局部坐标到平移向量场的映射，整体的层映射退化为一种扩散行为，本质上牺牲了表达能力(需要更多的层来表示旋转、仿射等变换)换取训练动力、稳定性与收敛性。
</blockquote>
</blockquote>
...
<blockquote class="section">
  ...
  DPN为我们打开了新思路，然而它的双流结构以直和的形式出现，是固定且无交互的。我们不妨再激进一点，如果网络能够自己学习某些层更适合残差连接、另外一些层更适合密集连接，那么就能在保证梯度流通的情况下通过更强的变换来提升特征表达能力。

  <blockquote class="subsection">
    
  ## HC的流与流混合
  我们不妨倒着从论文中给出的流数$n=4$去猜想，它代表了两条残差流与两条密集流的混合。
  </blockquote>
...
# 超连接网络的一般形式
在第三章，我们简单讨论了残差连接（ResNet）、密集连接（DenseNet）解决深度网络训练动力不足的思路，两者的方案都有些极端，后来的双流网络架构（DualPathNet）仍有一定局限。我们不妨考虑一种更灵活的方法：一种全局上自动权衡训练动力和特征表达的架构。
<blockquote class="section">
...
- \(p = 2\): 谱范数 (最大的**奇异值**) \(\|A\|_2 = \sigma_{\max}(A)\)  
...
| 步骤              | 内容                                                                                                               |
| --------------- | ---------------------------------------------------------------------------------------------------------------- |
| **输入**          | 一个任意实方阵 $\mathcal{\tilde H} \in \mathbb{R}^{n\times n}$，最大迭代步长$T$,容差$\epsilon$                          |
| **目标**          | 生成一个**双随机矩阵**的近似 $\mathcal{A}$                                                                        |
| **初始化**         | 通过逐元素指数映射，令 $X_{(0)} :=exp(\mathcal{\tilde H})$                                                                                        |
| **迭代（k=1,…,T）** | 交替进行行归一化与列归一化(可置换)：                                                                                                   |
|                 | 1) **行归一化**： $X_{(t+\frac{1}2)}=Diag(X_{(t)}\boldsymbol{1})X_{(t)}$ |
|                 | 2) **列归一化**：$X_{(t+1)}=X_{(t+\frac{1}2)}Diag(\boldsymbol{1}^TX_{(t+\frac{1}2)})$  |
| **终止条件**        | 行和误差与列和和误差(与1比较)均小于容差$\epsilon$，或迭代步数抵达最大上限$T$。                                                                               |
| **输出**          | 近似的双随机矩阵$\mathcal{A}$                                                                               |
...
## 实例分析：mHC中的双随机矩阵参数化
  我们首先回顾一下Xavier_uniform和Xavier_normal的定义，

  <blockquote class="definition">

  给定一个矩阵空间$\mathbb{R}^{r\times c}$,满足如下条件的矩阵$W\in \mathbb{R}^{r\times c}$具有前向/反向传播稳定性：
  $$Var(w_{ij})=\frac{2}{r+c}$$因为对于$$\begin{cases}
    y_i=\sum^{c}_{j=1}W_{ij}x_j\\
    \delta x_j=\sum^r_{i=1}W_{ij}\delta y_i\\
    x\in \mathbb{R}^{c\times 1},Var[x]=v_x
  \end{cases}$$所谓的传播稳定性是指前向传播的输出值方差、反向传播的梯度流量(反馈给上一层的余切量)方差尽可能只和输入无关而与网络、网络层数无关。
  不妨假设$W_{ij}$均值为0并独立于$x$，我们有$$\begin{cases}
  Var[y_i]=\sum_{j=1}^c Var[W_{ij}]Var[x_j]=c\cdot Var[W_{ij}]\cdot v_x\\
  Var[\delta x_j]=\sum_{i=1}^rVar[W_{ij}]Var[\delta y_i]=r\cdot Var[W_{ij}]\cdot v_{\delta y}
\end{cases}$$
  其中当前层余切量方差$v_{\delta y}$受输出方差$Var[y]$控制，输出方差受输入方差$v_x$控制。
  不难看出我们希望$Var[W_{ij}]=\frac{1}{c}$且$Var[W_{ij}]=\frac{1}{r}$,当$W$非方阵，显然取开头的调和平均是最折中的方案。
  </blockquote>

  <blockquote class="remark">
    
也许大家会有疑问，对于$y=Wx$我们的反向传播的梯度形式应该是
$$\frac{\partial \mathcal{L}}{\partial W}=\delta y \cdot x^T$$为什么上文关注的却是$$\delta x=W\delta y$$实际上我们考虑的是一个完整深度网络梯度流传播，每一层的$delta x$实际上是上一层的$\delta y$,我们关注的是梯度反向流经每一层网络时的波动性，而不是具体某一层的参数是如何被更新的。
  </blockquote>
  <blockquote class="example">
    
  对于服从均匀分布的随机变量$w_{ij}\sim Uniform(a,b)$，$a<b$其方差$$Var(w_{ij})=\frac{(b-a)^2}{12}$$,
  不妨令其中心化，也即$a+b=0$,则$$Var(w_{ij})=\frac{a^2}{3}$$
  由此得到该情形下$w_{ij}\sim Uniform(-\sqrt{\frac{6}{r+c}},\sqrt{\frac{6}{r+c}})$
同理，对于$w_{ij}\in \mathcal{N}(\mu,\sigma^2)$我们只需要令$\sigma^2=\frac{2}{r+c}$，选择一个均值后进行逆变换采样。

  </blockquote>
  <blockquote class="definition">
 
  $$\begin{cases}
  \vec{x}_l^{'}={RMSNorm} (Vec(\tilde x_l))\\
  \tilde{H}_l^{res}=\alpha_l^{res}\cdot mat((\vec{x}_l^{'})^T\phi_l^{res})+b_l^{res}
  \end{cases}$$
其中$\tilde x_l\in\mathbb{R}^{n\times d},Vec(·)$是按行展开，也即$\vec{x}_l^{'} \in \mathbb{R}^{nd\times 1}$；$\alpha_l^{res}\in \mathbb{R}$，$\phi_l^{res} \in\mathbb{R}^{nd\times n^2},b_l^{res}\in\mathbb{R}^{n\times n}$是模型参数。
为了简洁，下面忽略指示层数和类型的上下标，模型参数的初始值如下
$$\begin{cases}
  \alpha_0=0.01\\
  (\phi_0)_{ij}\sim Uniform(-\sqrt{\frac{6}{n^2+nd}},\sqrt{\frac{6}{n^2+nd}})\\
  b_0=I_n
\end{cases}$$
其中$I_n$是单位方阵。

双随机矩阵由$\tilde H_l^{res}$通过逐点指数映射后通过Sinkhorn算法迭代得到，
$$H^{res}_l={Sinkhorn}(exp(\tilde H_l^{res}))$$
  </blockquote>

<blockquote class="remark">
  
  我们现在只知道输入层的HyperFeature $\tilde x_0=\boldsymbol{1}_n x_0^T$，并且可以认为$x_0\sim\mathcal{N}(0,I_d)$，而$\vec{x}_l^{'}$实际上可以看作：$$Vec(\sqrt{nd}\cdot\frac{\tilde{x}_l}{\|\tilde{x}_l\|_F}),$$关于RMSNorm的讨论详见后文8.1部分对数值稳定层的介绍。
  显然我们可以断言$\vec{x}_0^{'}$不是一个各维度相互独立的随机向量。
</blockquote>

<blockquote class="proposal">
  
我们简记$y=( \vec{x}^{'})^T\phi$，$\phi_{ij}$独立同分布且独立于$\vec{x}^{'}$，默认下指标求和，有：$$\begin{cases}
  y_j=\vec{x}^{'}_i\phi_{ij}\\
  Var[y_j]=nd\cdot Var[\vec{x}^{'}_i]Var[\phi_{ij}]=nd\cdot Var[\vec{x}^{'}_i]\cdot\frac{2}{nd+n^2}
\end{cases}$$当$n=4$显然$n<<d$,有$Var[y_j]\approx 2\cdot Var[\vec{x}^{'}_i]$,
对于第0层，也即$$\vec{x}^{'}=Vec(\sqrt{nd}\cdot\frac{\boldsymbol{1}_n x^T}{\sqrt{n}\cdot\|x\|_2}),x_i\sim \mathcal{N}(0,1)$$有$$Var[\vec{x}^{'}_i]\approx 1,Var[y_j]\approx 2$$
</blockquote>
<blockquote class="remark">
  
  尽管在第0层我们可以合理假设扩张前特征$x$服从标准多元正态分布(中间层可能会受到扩张-压缩映射的低秩行为影响，难以确定服从何种分布)，理论上我们无法断言$y_j$服从何种分布。
  我们可以确定的是，第0层的$$\begin{cases}
    x_i\sim \mathcal{N}(0,1),\\
    \|x\|_2^2\sim\mathcal{X}_d^2,\\
    \vec{x}^{'}_i\sim t(d),
  \end{cases}$$当$d$较大时，大自由度t分布接近正态分布。
当$\phi_{ij}$服从均匀分布，其与t分布乘积没有闭式解，$Y_{j}$尾部较重；
当$\phi_{ij}$服从正态分布，根据CLT，其乘积分布收敛于正态分布，$Y_{j}$尾部较轻。
...
在前述章节中，我们已经讨论过双随机矩阵在深层网络中的意义：其作为一种**谱范数**为 1 的线性变换，在长复合映射链中起到了抑制梯度爆炸与消失的结构性作用。从这一角度看，Sinkhorn 算法可以被理解为一种离散的谱重整化过程，其目标不是单纯的数值归一，而是将矩阵推向一个谱特征被规范化的结构子空间。
...
我们将 Sinkhorn 算法视为定义在非负实矩阵锥 $\mathbb{R}_{\ge 0}^{n \times n}$ 上的一个离散重整化动力系统。其行为受到两个核心约束：
1. 矩阵内部的块结构与谱分离特性；
2. 非负性所诱导的 Perron–Frobenius 结构；
3. 模态竞争。
  
在介绍这些内容之前，我们先给出**谱半径**的定义，并将其与上文的**谱范数**进行区分。

<blockquote class="theorem">

**谱范数的行列和界**  
设 $A \in \mathbb{C}^{m \times n}$ 为任意矩阵，其谱范数（算子 2-范数）定义为
\[
\|A\|_2 := \max_{x \neq 0} \frac{\|Ax\|_2}{\|x\|_2}=\sigma_{max}(A)=\sqrt{\lambda_{max}(A^*A)}.
\]  
则谱范数满足以下上界：
\[
\|A\|_2 \le \sqrt{\|A\|_1 \, \|A\|_\infty},
\]
其中
\(\|A\|_1 = \max_j \sum_i |A_{ij}|\) 为列和最大值，
\(\|A\|_\infty = \max_i \sum_j |A_{ij}|\) 为行和最大值。

</blockquote>

<blockquote class="theorem">

对于**非负实方阵** $A \in \mathbb{R}_{\ge 0}^{n \times n}$，其谱半径
\[
...
**谱范数与谱半径：矩阵奇异值与方阵特征值**  
我们只能在方阵里同时讨论它的谱范数和谱半径。
- **谱范数** \(\|A\|_2\)：**矩阵**作用在所有向量方向上的最大伸缩，由最大奇异值定义，始终为实数。  
- **谱半径** \(\rho(A)\)：**方阵**特征值模的最大值，反映主模态方向的增长因子。 
- 对任意方阵，总有 \(\rho(A) \le \|A\|_2\)。  
- 当方阵是对称或正常时，谱范数等于谱半径；对于非对称或存在复特征值的矩阵，谱范数可能严格大于谱半径。  
...
**Perron–Frobenius**  
设 $A \in \mathbb{R}^{n \times n}$ 且 $A > 0$，则：

1. $A$ 存在唯一的最大特征值 $\lambda_1 = \rho(A) > 0$；
2. 对应特征向量 $x \in \mathbb{R}^n$ 严格正：
   $$
   A x = \lambda_1 x, \quad x_i > 0 \quad \forall i;
   $$
3. 所有其他特征值 $\lambda_i \in \sigma(A)$ 满足
   $$
   |\lambda_i| < \lambda_1, \quad i = 2, \dots, n;
   $$
4. $\lambda_1$ 的代数重数与几何重数均为 1。
...
**Perron–Frobenius**  
设 $A \in \mathbb{R}_{\ge 0}^{n \times n}$ 为非负不可约矩阵，则存在一个实数 $\lambda_{\mathrm{PF}} > 0$，称为 Perron–Frobenius 特征值，使得：

1. $\lambda_{\mathrm{PF}}$ 是 $A$ 的一个**实特征值**；
2. 对应特征向量 $x \in \mathbb{R}^n$ 可以取非负或严格正：
   $$
   A x = \lambda_{\mathrm{PF}} x, \quad x_i \ge 0, \ x \neq 0;
   $$  
3. 对任意其他特征值 $\lambda \in \sigma(A)$，都有
   $$
   |\lambda| \le \lambda_{\mathrm{PF}};
   $$  
4. 当 $A$ 为本原矩阵（primitive）时，$\lambda_{\mathrm{PF}}$ 的代数重数与几何重数均为 1。
...
# Sinkhorn-Knopp 算法收敛经验公式

本章讨论 Sinkhorn-Knopp 算法在正矩阵上的收敛行为、数值稳定性优化以及如何基于参数统计特性预测迭代表现。内容结合理论结果与实践经验，为实验设计提供参考。

<blockquote class="discussion">

## 1) 收敛性与迭代上界

设输入矩阵为严格正矩阵 \(A\in\mathbb{R}_+^{n\times n}\)，Sinkhorn-Knopp 算法通过交替行/列归一化生成双随机矩阵 \(X\in\mathcal{B}_n\)：

\[
\mathcal{B}_n = \{ X\in\mathbb{R}_+^{n\times n} \mid X\mathbf{1}_n = \mathbf{1}_n,\; X^\top \mathbf{1}_n = \mathbf{1}_n \}.
\]

迭代收敛性保证：
- 输入矩阵为正或完全不可分解时迭代线性收敛。
- 收敛速度受矩阵**有效密度**与**动态范围**影响，而非谱结构。

经验估计迭代次数：
\[
T \approx C_1 + C_2 \left( \log n + \log \frac{\max A_{ij}}{\min A_{ij}} - \log \varepsilon \right),
\]
其中 \(\varepsilon\) 为容忍误差，\(C_1,C_2\) 为经验常数。
...
训练中常用技巧：
1. **Log-domain Sinkhorn**：对数域更新迭代，避免 \(\exp(\tilde H)\) 下溢或上溢。
\[
L_{ij} \leftarrow L_{ij} - \operatorname{logsumexp}_j(L_{i:}),\quad
L_{ij} \leftarrow L_{ij} - \operatorname{logsumexp}_i(L_{:j})
\]
2. **加 \(\varepsilon\)**：输入矩阵 \(A \leftarrow A + \varepsilon\) 保证元素正。
3. **clip logits**：\(\tilde H \leftarrow \operatorname{clip}(\tilde H,-B,B)\)，控制动态范围。
4. **早停机制**：根据行/列和偏差 \(\delta = \max_i|r_i-1| \vee \max_j|c_j-1|\) 停止迭代。
...
假设
\[
\tilde H = \alpha \cdot (x' \phi) + b, \quad A = \exp(\tilde H),
\]
可提取以下统计特征：
- 均值 \(\mu\)、方差 \(\sigma^2\)  
- 最大/最小差 \(r=\max \tilde H - \min \tilde H\)  
- log-range \(R = e^r\)  
- 有效密度 \(\gamma(\rho) = \frac{1}{n^2}\sum_{i,j}\mathbf{1}\{A_{ij}\ge \rho\}\)  
- 行/列初始不均匀度（KL divergence 或方差）

启发式经验：
- 小 \(\sigma\)、小 \(r\) → 快速收敛；
- 大 \(\alpha\) 或大 log-range → 迭代更多；
- 高有效密度矩阵收敛更快。
...
1. 矩阵尺寸：\(n \in \{16,32,64\}\)  
2. 收敛容忍度 \(\varepsilon_{\text{tol}} = 10^{-3} \sim 10^{-6}\)  
3. 记录：迭代次数 \(T\)、初始特征 \((\sigma,r,\gamma)\)、最终矩阵 entropy 等  
4. 数值稳定实现：优先 log-domain + \(\varepsilon\) + clip  
5. 可绘制特征-迭代次数散点图或回归热力图，验证统计量与收敛时间关系。

---

## 5) 总结

- **谱分析作用有限**：算法非线性、交替归一化，收敛不由谱半径控制。  
- **关键因素**：矩阵元素分布、有效密度、动态范围。  
- **实践策略**：控制 \(\tilde H\) 的方差与尺度，使用 log-domain Sinkhorn 保证数值稳定，结合统计特征预测迭代表现。
...
|------|-------------|----------------------------|----------------------------|----------------------|
| **残差结构** | 残差连接（Pre-/Post-Norm） | Post-Norm 维持深层表示多样性；Pre-Norm 易发生表示塌缩 | Post-Norm 梯度消失严重；Pre-Norm 梯度稳定 | **稳定性–表达性跷跷板** |
...
# 位置编码
自然语言是有语序的，原生的Transformer结构的置换不变性需要被打破。
<blockquote class="appendix-section">

## 绝对位置编码
通过为词嵌入附加一个位置编码向量来打破置换不变性，但需要模型自己区分语义差异与位置差异，因为破坏了原始语义模长信息。
...
## 相对位置编码
发生在线性投影与注意力得分计算之间，通过旋转词嵌入向量(如RoPE)来调节注意力得分，不会混入高阶误差项。
...
# 文本空间与纤维表示
我们可以把一张二维图像看作一个二维环面，作为底空间，RGB等特征是其上纤维；一个带图层的三维图像可以看作一个三维环面，作为底空间......
那么自然语言的底空间是什么。
我们知道相对位置编码可以在表示层面帮助我们实现顺序关系，那么我们不妨定义文本空间为一个离散点集上的拓扑。
<blockquote class="definition">
...
显然，Tokenization是从平凡拓扑出发，也即给定一个初始词汇表$V_0=\set{\varnothing,\mathcal{A}}$,不断朝离散拓扑演化、扩张，在某个中间态停止。
</blockquote>
