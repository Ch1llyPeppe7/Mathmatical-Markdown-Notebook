...
受到深层网络结构影响的部分：$$\frac{\partial {x_L}}{\partial{x_{i+1}}}=\prod ^{L-1}_{j=i+1} \frac{\partial {x_{j+1}}}{\partial{x_{j}}}=\prod ^{L-1}_{j=i+1} s_j ,$$
代入$(8)、(9)$容易发现
...
其中，$X_i\subset\mathbb{R}^d$,$F_i$为含非线性变换网络作用,如$Transformer$中的$FFN$：
...
其中，$  X_i,X_{i+1}\subset X\subset \mathbb{R}^d,Z_1\subset Z_0\subset \mathbb{R}^D,d<D$,
...
\begin{align*}
    ...
    X_2\xrightarrow{F_3}&X_1^{\frac{3!}{2!}}\xrightarrow{F_4} ...\xrightarrow{F_{L-1}} X_1^{\frac{{(L-1)}!}{2!}},\\
...
<blockquote class="example">
  ...
  $$\begin{align*}
    ...
    &唯一不动点、最大内切球中心\mathcal{A}_{Uni}=\frac{1}{n}\boldsymbol{1}\boldsymbol{1}^T\\
  \end{align*}$$
